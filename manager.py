...
# calculates sum for each activity
print(sums, labels)
explode = [0.1]*len(sums)
# explode is the distance from the centre
...
# calculates the sum for various modes of transport
explode = [0.1]*len(sums)
# explode calculates the distance from the centre
fig1, ax1 = plt.subplots()
#autopct enables you to display the percent value using Python string formatting.
ax1.pie(sums, explode=explode, labels=labels, autopct='%1.1f%%',
        shadow=True, startangle=0)
# this creates a pie chart based on the calculated data
ax1.axis('equal')
...
# calculates the amount spent on each category in each city of europe
sums = list(zip(*sums))
...
# Then we create a bar graph from the calculated data
...
for d in list(all_dates):
    ...
    # calculates the value of money spent each day
    if value:
        # if value is present then appends it to the corresponding date
        cities.append(df[df['date'] == d.date()]['city'].values[-1])
        daily_expenses.append((d.date(), value))
    else:
        # else it removes the date with 0 as its value
        all_dates.remove(d)
...
plt.show()
# then we create a bar graph with the calculated daily expense and corresponding date
